...
import {
    ExecutorDeployConfig,
    ExecutorDeployArtifacts,
    ExecutorDeployedContracts,
    ContractsDeployment
...
contract DeployExecutor is Script {
    ...
    using ExecutorDeployArtifacts for ExecutorDeployArtifacts.Context;
    using ExecutorDeployedContracts for ExecutorDeployedContracts.Context;

    error ChainIdMismatch(uint256 actual, uint256 expected);

    ExecutorDeployArtifacts.Context internal _deployArtifact;

    function run() public {
        string memory configFileName = vm.envString("DEPLOY_CONFIG_FILE_NAME");
        console.log("Loading config file: %s", configFileName);

        _deployArtifact.deployConfig = ExecutorDeployConfig.load(DeployFiles.resolveDeployConfig(configFileName), "");
        ...
        Executor executor = ContractsDeployment.deployExecutor(_deployArtifact.deployConfig.owner);

        _deployArtifact.deployedContracts = ExecutorDeployedContracts.Context({executor: executor});
