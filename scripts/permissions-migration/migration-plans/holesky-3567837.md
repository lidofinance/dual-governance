...
38. Revoke RUN_SCRIPT_ROLE permission from Voting on Agent
39. Set RUN_SCRIPT_ROLE manager to Agent on Agent
40. Grant EXECUTE_ROLE permission to DGExecutor on Agent
41. Revoke EXECUTE_ROLE permission from Voting on Agent
42. Set EXECUTE_ROLE manager to Agent on Agent
```
...
```
45. Grant PAUSE_ROLE to ResealManager on ValidatorsExitBusOracle
46. Grant RESUME_ROLE to ResealManager on ValidatorsExitBusOracle
```
...
```
47. Grant DEFAULT_ADMIN_ROLE to Voting on AllowedTokensRegistry
48. Revoke DEFAULT_ADMIN_ROLE from Agent on AllowedTokensRegistry
49. Grant ADD_TOKEN_TO_ALLOWED_LIST_ROLE to Voting on AllowedTokensRegistry
50. Revoke ADD_TOKEN_TO_ALLOWED_LIST_ROLE from Agent on AllowedTokensRegistry
51. Grant REMOVE_TOKEN_FROM_ALLOWED_LIST_ROLE to Voting on AllowedTokensRegistry
