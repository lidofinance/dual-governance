...
contract LaunchAcceptanceDAO is DGDeployArtifactLoader {
    ...
    function run() external {
        ...
        address daoEmergencyGovernance = address(dgLaunchConfig.daoEmergencyGovernance);

        if (fromStep == 0) {
            DeployVerification.verify(_deployArtifact);
            // Replace the emergency governance in the Timelock with the DAO Emergency Governance to simulate the emergency mode
            // with Lido DAO Voting process
            vm.prank(address(_deployArtifact.deployedContracts.adminExecutor));
            timelock.setEmergencyGovernance(daoEmergencyGovernance);
        } else {
        ...
        if (fromStep <= 3) {
            ...
            require(
                timelock.getGovernance() == daoEmergencyGovernance,
            ...
            ExternalCallsBuilder.Context memory builder = ExternalCallsBuilder.create(5);
            ...
            CallsScriptBuilder.Context memory voteWithProposal = CallsScriptBuilder.create(
                daoEmergencyGovernance,
                abi.encodeCall(
                    _deployArtifact.deployedContracts.dualGovernance.submitProposal,
                    (builder.getResult(), "Proposal submitted by the DAO")
                )
            );

            console.log("Submitting DAO proposal to reset Dual Governance to the initial state");
            uint256 voteId = _lidoUtils.adoptVote(
                "Resetting all Dual Governance params to the initial state", voteWithProposal.getResult()
            );
            _lidoUtils.executeVote(voteId);
            console.log("Vote executed with ID", voteId);
            console.log("DG Proposal submitted");

            console.log("DG Proposal ID");
        } else {
        ...
        if (fromStep <= 4) {
            ...
            dgLaunchConfig.daoEmergencyGovernance.scheduleProposal(proposalId);
        ...
        if (fromStep <= 6) {
            ...
            require(
                timelock.getEmergencyGovernance() == daoEmergencyGovernance,
