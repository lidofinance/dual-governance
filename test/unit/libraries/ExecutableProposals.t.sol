...
contract ExecutableProposalsUnitTests is UnitTest {
    ...
    function test_submit_reverts_if_empty_proposals() external {
        ...
        _proposals.submit(proposer, address(0), new ExternalCall[](0), "Empty calls");
    }
    ...
    function test_submit_proposal() external {
        ...
        vm.expectEmit();
        emit ExecutableProposals.ProposalSubmitted(expectedProposalId, proposer, address(_executor), calls, description);

        vm.recordLogs();

        _proposals.submit(proposer, address(_executor), calls, description);
    ...
    function test_get_proposal_info_and_external_calls() external {
        ...
        _proposals.submit(proposer, address(_executor), expectedCalls, "");
        uint256 proposalId = _proposals.getProposalsCount();
