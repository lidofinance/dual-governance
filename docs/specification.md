...
The emergency mode lasts up to the **emergency mode max duration** counting from the moment of its activation. While it's active, 1) only the **emergency execution committee** has the right to execute scheduled proposals, and 2) the same committee has the one-off right to **disable the DG subsystem** (after that action the system should start behaving in order to [this specification](plan-b.md)), i.e. disconnect executor contracts from the DG contracts and reconnect them to the SG contract. The latter also disables the emergency mode and the emergency execution committee, so any proposal can be executed by the DAO without cooperation from any other actors.
